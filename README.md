...
- Game install: `~\AppData\Roaming\Vintagestory` (contains `VintagestoryAPI.dll`,
  `VintagestoryLib.dll`, and `Mods\VSSurvivalMod.dll` / `Mods\VSEssentials.dll` - referenced
  directly by absolute path in the csproj).
- Game data / save / installed mods: `~\AppData\Roaming\VintagestoryData`
  (mods go in `VintagestoryData\Mods`, per-mod config in `VintagestoryData\ModConfig`, logs in
  `VintagestoryData\Logs`).
- That VintagestoryData folder is itself a separate git repo (a curated mod list, with its own
  AGENTS.md) - unrelated to this repo except that Toolsmith and this addon both end up installed
  in its `Mods\` folder for testing. Never write into that repo from here.
